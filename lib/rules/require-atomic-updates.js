...
function isLocalVariableWithoutEscape(variable) {
    const [start, end] = variable.scope.variableScope.block.range;

    return variable.references.every(reference => {
        const scope = reference.from.variableScope.block;

        return scope.range[0] >= start && scope.range[1] <= end;
    });
}
        ...
        return {
            ...
            Identifier(node) {
                ...
                if (writeExpr &&
                    ...
                    variable.scope.variableScope !== scope
                ) {
                    console.log("register: %o by %o (%o)", variable.name, sourceCode.getText(writeExpr.parent), sourceCode.getText(reference.identifier));
            ...
            ":expression:exit"(node) {
                ...
                if (references) {
                    console.log("write: %o (%o)", sourceCode.getText(node.parent), sourceCode.getText(node));
                    assignmentReferences.delete(node);

                    for (const reference of references) {
                        console.log("reference: %o", sourceCode.getText(reference.identifier));
                        const variable = getVariable(reference);
